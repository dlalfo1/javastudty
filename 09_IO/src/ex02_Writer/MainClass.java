...
public class MainClass {
	...
	*/
	...
	public static void ex01() {
		...
		int c = 'I'; // 문자를 int에 저장할 수 있다. char도 정수타입이고 int가 4바이트 char가 2바이트기 때문.
		char[] cbuf = {' ', 'a', 'm'}; 
		String str = " a boy.";				
		...
		} finally { // 예외 발생여부와 상관없이 반드시 마지막에 실행되는 영역이다
			try {
				if(fw != null) { 
	...
	public static void ex02() { // ex01 연습
		...
		try {
			...
			System.out.println("ex02.txt 파일이 생성되었다.");
		} catch (IOException e) {
			e.printStackTrace(); // 예외발생 당시의 호출스택에 있었던 메서드의 정보와 예외 메시지를 출력
		} finally {
			try {
				if(fw != null)
				fw.close();
	...
	*/
	...
	try (FileWriter fw = new FileWriter(file)){ 
		...
		fw.write(cbuf, 0, 2); // 인덱스 0부터 2글자만  보내시오. -> 즉 a,b 만 보내라
		fw.write(str, 2, 3); // 인덱스 2부터 3글자만 보내시오. -> 즉 c,d,e만 보내라
		
	} catch (IOException e) {
		e.printStackTrace();
	}

	} // 자동으로 스트림 닫아주니까 close 쓸 필요없음. 그럼 이걸로 하면 되는거 아닌가요...
	...
	*/
		...
		System.out.println("ex05.txt 파일이 생성되었다");
			...
			out.close(); // close 메소드 내부에 이미 IOException을 처리하고 있다.
		}
	}
