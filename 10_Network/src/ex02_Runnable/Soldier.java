...
/*
	 ...
	 2. Runnable 인터페이스의 public abstract void run(); 추상메소드를 오버라이드 한다. 
  	 3. Runnable 객체를 만들고, Runnable 객체를 이요해서 Thread 객체를 만든다.
...
public class Soldier implements Runnable{
	...
	private String name;
	private Gun gun; // Gun을 가지고 있더라도 private 필드값을 가져오려면 게터,세터를 이용해서 가져와야 한다.
	...
	public void run() {
		...
		try {
			...
			while(gun.getBullet() != 0) {
				shoot();
				Thread.sleep(1000); // 컴퓨터가 실제로 1초간 멈춘다.
			}
